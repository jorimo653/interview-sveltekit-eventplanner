    ...
    <div class="card-body flex items-center">
        ...
        {#if isListCard}
            <div class="card-actions flex w-full justify-evenly">
                <a href="/editevent/{eventId}" class="btn btn-outline btn-neutral flex flex-1">Edit</a>
                <form method="POST" action="?/deleteEvent" class="btn btn-outline btn-warning flex flex-1">
        ...
        {:else}
            <div class="card-actions flex w-full">
                <a class="btn btn-outline btn-primary flex flex-1" href="/" role="button">Back to Home</a>
            </div>
        {/if}
    </div>
</div>